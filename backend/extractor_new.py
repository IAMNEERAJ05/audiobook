...
class PDFExtractor:
    ...
    def extract_pages(self, pdf_path: str) -> list:
        ...
        # When frozen, copy PDF to a fast local temp to avoid cloud path slowness
        working_pdf_path = pdf_path
        if getattr(sys, 'frozen', False):
            try:
                temp_dir = Path(tempfile.gettempdir()) / "AudiobookGenerator"
                temp_dir.mkdir(exist_ok=True)
                dest = temp_dir / Path(pdf_path).name
                if not dest.exists() or os.path.getmtime(pdf_path) > os.path.getmtime(dest):
                    shutil.copy2(pdf_path, str(dest))
                working_pdf_path = str(dest)
            except Exception:
                working_pdf_path = pdf_path
