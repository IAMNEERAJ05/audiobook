...
class DatabaseConnection:
    ...
    def __init__(self, db_path=None):
        if self._connection is None:
            if db_path is None:
                # Use a distinct DB path when frozen to separate from dev runs
                if getattr(sys, 'frozen', False):
                    appdata = Path(os.getenv('APPDATA') or (Path.home() / 'AppData' / 'Roaming'))
                    app_data_dir = appdata / "AudiobookGenerator"
                else:
                    app_data_dir = Path.home() / ".audiobook_generator"
                app_data_dir.mkdir(exist_ok=True)
                db_path = str(app_data_dir / "audiobooks.db")
