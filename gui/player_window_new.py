...
class PlayerWindow(QWidget):
    ...
    def __init__(self, book_id=None, manifest=None, parent=None):
        super().__init__()
        ...
        self.setFixedSize(800, 600)  # Fixed size, non-resizable
    ...
    def init_ui(self):
        ...
        # Back button (only show if parent exists)
        if self.parent_window:
            back_layout = QHBoxLayout()
            self.back_btn = QPushButton("← Back to Library")
            self.back_btn.clicked.connect(self.go_back_to_library)
            ...
            main_vertical_layout.addLayout(back_layout)
        ...
        # Add main layout to vertical layout
        main_vertical_layout.addLayout(main_layout)
        self.setLayout(main_vertical_layout)
    
    def go_back_to_library(self):
        """Go back to library window."""
        if self.parent_window:
            self.hide()  # Hide this window
            self.parent_window.show()  # Show library window
