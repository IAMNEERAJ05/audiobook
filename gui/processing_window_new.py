...
class ProcessingWindow(QWidget):
    ...
    def __init__(self, pdf_path=None, parent=None):
        ...
        self.setFixedSize(800, 700)  # Fixed size, non-resizable
        self.pdf_path = pdf_path
        ...
        # Flag to indicate processing finished normally
    ...
    def closeEvent(self, event):
        ...
        # If processing finished normally, accept the close and do nothing else.
        if getattr(self, "processing_complete", False):
            event.accept()
            return
            ...
            if self.worker and self.worker.isRunning():
                ...
                if reply == QMessageBox.Yes:
                    try:
                        ...
                        self.worker.wait()
                        # Delete the book entry when closing during processing
                        try:
                            self.audiobook_service.delete_audiobook(self.book_id)
                            self.status_log.add_info(f"Deleted book entry for {self.book_id} due to window close")
                        except Exception as e:
                            self.status_log.add_error(f"Error deleting book on close: {str(e)}")
                    except Exception:
                        pass
                else:
                    event.ignore()
        ...
        # Show parent window if available and not finishing due to processing completion
        if self.parent_window and not getattr(self, "processing_complete", False):
            try:
                self.parent_window.show()
            except Exception:
                pass
        # Hide this window and ignore the close event (preserve app state)
        self.hide()
        event.ignore()
    ...
    def start_processing(self):
        ...
        # Check if book already exists and delete it
        existing_book = self.audiobook_service.get_audiobook_info(self.book_id)
        if existing_book:
            self.audiobook_service.delete_audiobook(self.book_id)
            self.status_log.add_info(f"Removed existing entry for {self.book_id}")
    ...
    def processing_finished(self):
        ...
        # Mark processing as complete so closeEvent knows not to reopen parent
        self.processing_complete = True

        # Open the library window and hide the home window
        if self.parent_window:
            try:
                from gui.library_window_new import LibraryWindow
                self.library_window = LibraryWindow(parent=self.parent_window)
                self.parent_window.hide()  # Hide home window
                self.library_window.show()  # Show library window
            except Exception as e:
                # If library fails to open, still allow closing
                self.status_log.add_error(f"Failed to open library window: {e}")
        
        # Close this window now (closeEvent will accept because processing_complete is True)
        self.close()
    ...
    def cancel_processing(self):
        ...
        # Delete the book entry from database
        try:
            if self.audiobook_service.delete_audiobook(self.book_id):
                self.status_log.add_success(f"Cancelled processing and deleted book entry: {self.book_id}")
                QMessageBox.information(self, "Processing Cancelled", 
                                      "Processing cancelled successfully.\nBook entry has been removed from database.")
            else:
                self.status_log.add_warning("Failed to delete book entry from database")
        except Exception as e:
            self.status_log.add_error(f"Error deleting book: {str(e)}")
